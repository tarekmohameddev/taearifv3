...
export const getDefaultApplicationFormData = (): ComponentData => ({
  ...
  header: {
    ...
    description: "املأ البيانات المطلوبة لتقديم طلب المعاينة. سيتم التواصل معك قريبًا لترتيب موعد لزيارة العقار ومعاينته بشكل دقيق.",
    textAlign: "text-center",
    marginBottom: "mb-8",
    typography: {
      title: {
        className: "font-bold text-[20px] text-emerald-600 md:text-[32px] leading-[22.32px] md:leading-[35.71px]",
        fontSize: "text-[20px] md:text-[32px]",
        fontWeight: "font-bold",
        color: "text-emerald-600",
      },
      description: {
        className: "font-normal text-[16px] leading-[17.86px] text-gray-600 md:text-[20px] md:leading-[22.32px]",
  ...
  form: {
    fields: {
      ...
      phone: {
        ...
        required: true,
        type: "tel",
        className: "text-[16px] placeholder:text-end font-medium text-gray-600 rounded-md border border-gray-300 p-2 outline-none focus:border-emerald-600",
      },
      address: {
        label: "العنوان",
    ...
    submitButton: {
      text: "رفع المعاينة",
      className: "rounded-md bg-emerald-600 text-white font-semibold text-[16px] py-2 w-full md:w-[400px] mx-auto mb-10 hover:bg-emerald-700 transition-colors",
    },
    imageUpload: {
      label: "تحميل صور العقار",
      description: "يرجى تحميل 3 صور واضحة للعقار، بما في ذلك واجهته الداخلية والخارجية. يُفضل أن تكون الصور بأعلى جودة لتسهيل عملية المعاينة",
      maxImages: 3,
...
export const applicationFormFunctions = {
  ...
  updateByPath: (state: any, variantId: string, path: string, value: any) => {
    const currentData = state.applicationFormStates?.[variantId] || getDefaultApplicationFormData();
    const segments = path
      .replace(/\[(\d+)\]/g, ".$1")
      .split(".")
      .filter(Boolean);
