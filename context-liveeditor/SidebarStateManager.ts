...
interface SidebarStateManager {
  ...
  setSelectedComponent: (component: ComponentInstanceWithPosition | null) => void;
...
export const useSidebarStateManager = create<SidebarStateManager>((set, get) => ({
  ...
  updateComponentData: (componentId, path, value) => {
    ...
    const pageComponents = editorStore.pageComponentsByPage[currentPage] || [];
    
    // Find and update the component
    const updatedComponents = pageComponents.map((comp: ComponentInstanceWithPosition) => {
  ...
  getComponentData: (componentId) => {
    ...
    const currentPage = get().currentPage;
    const pageComponents = editorStore.pageComponentsByPage[currentPage] || [];
    const component = pageComponents.find((comp: ComponentInstanceWithPosition) => comp.id === componentId);
...
function updateDataByPath(data: any, path: string, value: any): any {
  ...
  for (let i = 0; i < segments.length - 1; i++) {
    ...
    if (existing == null || typeof existing === 'string' || typeof existing === 'number' || typeof existing === 'boolean') {
      cursor[key] = nextIsIndex ? [] : {};
    } else if (Array.isArray(existing) && !nextIsIndex) {
      cursor[key] = {};
    } else if (typeof existing === 'object' && !Array.isArray(existing) && nextIsIndex) {
      cursor[key] = [];
    }
    cursor = cursor[key];
  }
