...
interface CrmStore {
  ...
  setNewStage: (stage: { name: string; description: string; color: string; iconName: string }) => void
  updateNewStage: (updates: Partial<{ name: string; description: string; color: string; iconName: string }>) => void
}
  ...
  persist(
    (set, get) => ({
      ...
      updateCustomerStage: (customerId, newStageId) => {
        ...
        if (customer) {
          updatedCache.set(customerId, { ...customer, pipelineStage: newStageId })
        }
      ...
      getCustomersByStage: (stageId) => {
        const { customers } = get()
        return customers.filter(customer => customer.pipelineStage === stageId)
      },
      ...
      getRemindersByCustomer: (customerId) => {
        const { reminders } = get()
        return reminders.filter(reminder => reminder.customer_id === customerId)
      },
      ...
      setShowAddAppointmentDialog: (show) => set({ showAddAppointmentDialog: show }),
      setShowEditAppointmentDialog: (show) => set({ showEditAppointmentDialog: show }),
      setShowAppointmentDetailDialog: (show) => set({ showAppointmentDetailDialog: show }),
      setShowAddNoteDialog: (show) => set({ showAddNoteDialog: show }),
      setShowAddReminderDialog: (show) => set({ showAddReminderDialog: show }),
      setShowEditReminderDialog: (show) => set({ showEditReminderDialog: show }),
      setShowAddInteractionDialog: (show) => set({ showAddInteractionDialog: show }),
