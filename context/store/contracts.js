...
const useContractsStore = create((set, get) => ({
  ...
  setContractStatusFilter: (contractStatusFilter) => set({ contractStatusFilter, currentPage: 1 }),
  setPaymentStatusFilter: (paymentStatusFilter) => set({ paymentStatusFilter, currentPage: 1 }),
  setRentalMethodFilter: (rentalMethodFilter) => set({ rentalMethodFilter, currentPage: 1 }),
    ...
    try {
      ...
      if (response.data.status) {
        ...
        const summary = {
          ...
          total_amount_due: data.reduce((sum, item) => sum + (item.rent_amount || 0), 0),
  ...
  getFilteredData: () => {
    const { contractsData, searchTerm, contractStatusFilter, paymentStatusFilter } = get();
    ...
    return contractsData.filter((item) => {
      ...
      // البحث في النص
      const matchesSearch = 
        (item.tenant_information?.tenant_name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      const matchesPaymentStatus = paymentStatusFilter === 'all' || 
        item.payment_status_color === paymentStatusFilter;
