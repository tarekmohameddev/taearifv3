export function getDeepScrollPosition(element: HTMLElement) {
  console.log('🔍 [DOM-UTILS] getDeepScrollPosition called for element:', element);
  let x = 0;
  let y = 0;
...
export function accumulateTransform(element: HTMLElement | null) {
  console.log('🔍 [DOM-UTILS] accumulateTransform called for element:', element);
  let scaleX = 1;
  let scaleY = 1;

  if (!element) {
    console.log('🔍 [DOM-UTILS] accumulateTransform - no element, returning default');
