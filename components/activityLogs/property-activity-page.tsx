...
export default function PropertyActivityLogPage() {
  ...
  const fetchPropertyLogs = async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<PropertyLogsResponse>(`/v1/properties/${propertyId}/logs`, {
        params: {
          page: currentPage,
          search: searchQuery,
  ...
  const createdCount = propertyLogs.filter(log => log.action === 'created').length;
  const updatedCount = propertyLogs.filter(log => log.action === 'updated').length;
  const deletedCount = propertyLogs.filter(log => log.action === 'deleted').length;
  const customCount = propertyLogs.filter(log => log.action === 'custom').length;
            ...
            <div className="mb-8">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h1 className="text-3xl font-bold text-gray-900">سجل نشاطات العقار</h1>
                </div>
              </div>
              <p className="text-gray-600">مراقبة وتتبع جميع الأنشطة المتعلقة بهذا العقار</p>
            </div>
                ...
                {loading ? (
                  <div className="flex items-center justify-center py-12">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-gray-600 font-medium">جاري تحميل سجل النشاطات...</span>
                    </div>
                  </div>
                ) : error ? (
