...
export default function ProjectActivityLogPage() {
  ...
  const fetchProjectLogs = async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<ProjectLogsResponse>(`/v1/projects/${projectId}/logs`, {
        params: {
          page: currentPage,
          search: searchQuery,
  ...
  const createdCount = projectLogs.filter(log => log.action === 'created').length;
  const updatedCount = projectLogs.filter(log => log.action === 'updated').length;
  const deletedCount = projectLogs.filter(log => log.action === 'deleted').length;
  const customCount = projectLogs.filter(log => log.action === 'custom').length;

  // Effects
  useEffect(() => {
    fetchProjectLogs();
  }, [projectId, currentPage, searchQuery, actionFilter]);
            ...
            <div className="mb-8">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h1 className="text-3xl font-bold text-gray-900">سجل نشاطات المشروع</h1>
                </div>
              </div>
              <p className="text-gray-600">مراقبة وتتبع جميع الأنشطة المتعلقة بهذا المشروع</p>
            </div>
                ...
                {loading ? (
                  <div className="flex items-center justify-center py-12">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-gray-600 font-medium">جاري تحميل سجل نشاطات المشروع...</span>
                    </div>
                  </div>
                ) : error ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">لا توجد سجلات نشاط للمشروع</h3>
