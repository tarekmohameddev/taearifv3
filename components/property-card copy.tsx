    ...
    >
      <div className="h-full w-full transition-transform hover:scale-[1.02]">
        ...
        <div className="relative w-full" style={{ aspectRatio: "16 / 10" }}>
          <Image
            src={p.image || "/placeholder.svg?height=300&width=480&query=property"}
            alt={p.title}
            fill
          ...
          <div className="absolute right-3 top-3 flex items-center gap-2 rounded-lg bg-white/95 px-3 py-2 shadow-sm">
            {p.bedrooms && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-semibold text-gray-700">{p.bedrooms}</span>
              </div>
            )}
            <div className="flex items-center gap-1">
              <span className="text-sm font-semibold text-gray-700">{p.views}</span>
          ...
          {isUnavailable && (
            <div aria-label="غير متاحة" className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/30" style={{pointerEvents:'none'}}>
              <span className="rounded-lg bg-white/90 px-4 py-2 text-sm font-bold text-gray-800" style={{pointerEvents:'none'}}>غير متاحة</span>
            </div>
          )}
        </div>
        ...
        <div className="mt-4 space-y-3" dir="rtl">
          ...
          <p className="line-clamp-2 text-sm text-muted-foreground">{p.district}</p>

          <div className="flex items-center justify-between">
            {/* السعر على اليسار */}
            <span className="text-lg font-extrabold text-foreground">{p.price} ريال</span>
            {/* زر التفاصيل على اليمين - بدون Link لتجنب التداخل */}
            <Button variant="ghost" className="h-auto p-0 text-emerald-700 hover:bg-transparent hover:underline">
              تفاصيل
            </Button>
          </div>
        </div>
