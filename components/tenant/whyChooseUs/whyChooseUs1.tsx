...
const getDefaultWhyChooseUsData = () => ({
  ...
  features: {
    ...
    card: {
      className: "rounded-2xl border bg-white p-6 shadow-sm ring-1 ring-emerald-50",
...
export default function WhyChooseUsSection(props: WhyChooseUsProps = {}) {
  ...
  const storeData = props.useStore ? (getComponentData('whyChooseUs', variantId) || {}) : {}
    ...
    for (const [pageSlug, pageComponents] of Object.entries(tenantData.componentSettings)) {
      ...
      if (typeof pageComponents === 'object' && !Array.isArray(pageComponents)) {
        ...
        for (const [componentId, component] of Object.entries(pageComponents as any)) {
          ...
          if ((component as any).type === 'whyChooseUs' && 
  ...
  return (
    <section 
      ...
      style={{ 
        backgroundColor: mergedData.background?.color || mergedData.styling?.bgColor || mergedData.colors?.background || "#ffffff",
    ...
    >
      ...
      >
        <header className={`${mergedData.header?.marginBottom || "mb-10"} ${mergedData.header?.textAlign || "text-right"} ${mergedData.header?.paddingX || "px-5"}`}>
          <h2 
            className={mergedData.header?.typography?.title?.className || "section-title text-right"}
            style={{
          ...
          <p 
            className={mergedData.header?.typography?.description?.className || "section-subtitle-xl"}
            style={{
              color: mergedData.styling?.textColor || mergedData.colors?.textColor || undefined
            }}
          >
            {mergedData.header?.description || "مكتبنا يجمع بين الخبرة والالتزام لتقديم خدمات مميزة في مجال العقارات"}
          </p>
        </header>
        ...
        >
          {(mergedData.features?.list || []).map((f: any, i: number) => (
            <article 
              key={i} 
              className={mergedData.features?.card?.className || "rounded-2xl border bg-white p-6 shadow-sm ring-1 ring-emerald-50"}
              style={{
                backgroundColor: mergedData.colors?.cardBackground || "#ffffff",
                borderColor: mergedData.colors?.borderColor || "#e5e7eb"
              }}
            >
              <div className={mergedData.features?.icon?.container?.className || "mx-auto flex size-20 items-center justify-center"}>
                <img 
                  src={f.icon} 
                  alt={f.title} 
                  className={mergedData.features?.icon?.image?.className || "h-[7rem] w-[7rem]"} 
                />
              </div>
              <h3 
                className={mergedData.features?.typography?.title?.className || "mt-6 text-center text-lg font-bold text-emerald-700"}
                style={{ color: mergedData.colors?.titleColor || "#059669" }}
              >
                {f.title}
              </h3>
              <p 
                className={mergedData.features?.typography?.description?.className || "mt-3 text-center text-lg leading-7 text-gray-600"}
                style={{ color: mergedData.colors?.descriptionColor || "#4b5563" }}
              >
                {f.desc}
              </p>
            </article>
