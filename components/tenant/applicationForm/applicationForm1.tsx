...
export default function ApplicationFormSection(props: ApplicationFormProps = {}) {
  // تحديد معرف المكون
      ...
      <div dir={mergedData.layout?.direction || "rtl"}>
          ...
          >
            ...
            >
              ...
              {/* العنوان والوصف */}
              <div className={`flex flex-col gap-y-4 ${mergedData.header?.textAlign || "text-center"} ${mergedData.header?.marginBottom || "mb-8"}`}>
                <h1 
                  className={mergedData.header?.typography?.title?.className || "font-bold text-[20px] text-emerald-600 md:text-[32px] leading-[22.32px] md:leading-[35.71px] text-center"}
                  style={{
                    color: mergedData.styling?.textColor || mergedData.styling?.focusColor || "#059669",
                  }}
                >
                  {mergedData.header?.title || "نموذج طلب معاينة"}
                </h1>
                <p className={mergedData.header?.typography?.description?.className || "font-normal text-[16px] leading-[17.86px] text-gray-600 md:text-[20px] md:leading-[22.32px] text-center"}>
                  {mergedData.header?.description || "املأ البيانات المطلوبة لتقديم طلب المعاينة. سيتم التواصل معك قريبًا لترتيب موعد لزيارة العقار ومعاينته بشكل دقيق."}
                </p>
              </div>

              {/* النموذج */}
              <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-y-4 md:gap-y-8 mt-10 md:mt-8 flex-wrap">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                  <div className="flex flex-col gap-y-4 w-full">
                    <label 
                      ...
                      style={{ color: mergedData.styling?.focusColor || "#059669" }}
                    >
                      {mergedData.form?.fields?.name?.label || "الاسم"}
                    </label>
                    <input
                      placeholder={mergedData.form?.fields?.name?.placeholder || "ادخل الاسم"}
                      value={formData.name}
                      onChange={(e) => handleInputChange("name", e.target.value)}
                      className={mergedData.form?.fields?.name?.className || "text-[16px] font-medium text-gray-600 rounded-md border border-gray-300 p-2 outline-none focus:border-emerald-600"}
                      type={mergedData.form?.fields?.name?.type || "text"}
                      name="name"
                      required={mergedData.form?.fields?.name?.required || true}
                      style={{
                        borderColor: mergedData.styling?.borderColor || "#d1d5db",
                        "--focus-color": mergedData.styling?.focusColor || "#059669",
                  ...
                  <div className="flex flex-col gap-y-4 w-full">
                    ...
                    <input
                      placeholder={mergedData.form?.fields?.phone?.placeholder || "ادخل رقم الهاتف"}
                      value={formData.phone}
                      onChange={(e) => handleInputChange("phone", e.target.value)}
                      className={mergedData.form?.fields?.phone?.className || "text-[16px] placeholder:text-end font-medium text-gray-600 rounded-md border border-gray-300 p-2 outline-none focus:border-emerald-600"}
                  ...
                  <div className="flex flex-col gap-y-4 w-full">
                    <label className="text-emerald-600 text-[16px] font-semibold">العنوان</label>
                    <input
                  ...
                  <div className="flex flex-col gap-y-4 w-full">
                    <label className="text-emerald-600 text-[16px] font-semibold">الملكية</label>
                    <input
                      placeholder="ادخل الملكية"
                      value={formData.requester_type}
                      onChange={(e) => handleInputChange("requester_type", e.target.value)}
                      className="text-[16px] font-medium text-gray-600 rounded-md border border-gray-300 p-2 outline-none focus:border-emerald-600"
                      type="text"
                      name="requester_type"
                      required
                 ...
                 <div className="flex flex-col lg:flex-row gap-y-4 gap-x-4 w-full">
                   <div className="flex-1 flex flex-col gap-y-4 relative">
                     <label className="text-emerald-600 text-[16px] font-semibold">التاريخ</label>
                     <Popover>
                       <PopoverTrigger asChild>
                         ...
                         >
                           ...
                           {selectedDate ? (
                             ...
                             <span className="text-gray-500">من فضلك اختر التاريخ اولا لاظهار الوقت</span>
                           )}
                         </Button>
                       </PopoverTrigger>
                  ...
                  <div className="flex-1 flex flex-col gap-y-4">
                    <label htmlFor="time" className="text-emerald-600 text-[16px] font-semibold">الوقت</label>
                    <div className="w-full relative">
                      ...
                      <div className="absolute pointer-events-none top-0 bottom-0 right-3 flex items-center order-2">
                        <svg className="w-5 h-5 text-emerald-600" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                          <path fillRule="evenodd" d="M12 1C5.925 1 1 5.925 1 12s4.925 11 11 11 11-4.925 11-11S18.075 1 12 1ZM11 6a1 1 0 1 1 2 0v5.586l2.707 2.707a1 1 0 1 1-1.414 1.414l-3-3A1 1 0 0 1 11 12V6Z" clipRule="evenodd" />
                        </svg>
                      </div>
                      {!isTimeEnabled && (
                ...
                {/* نوع العرض */}
                <div className="flex flex-col gap-y-4 w-full">
                  <label className="text-emerald-600 text-[16px] font-semibold">نوع العرض</label>
                  <div className="flex flex-row items-center justify-start gap-x-5">
                    <label className="flex items-center gap-x-4">
                      <input
                        ...
                        required
                      />
                      <span className="text-gray-600 text-[16px] font-bold">للبيع</span>
                    </label>
                    <label className="flex items-center gap-x-4">
                      <input
                        className="appearance-none border-2 border-emerald-600 rounded-full w-4 h-4 checked:bg-emerald-600 checked:border-emerald-600 focus:outline-none"
                        ...
                        required
                      />
                      <span className="text-gray-600 text-[16px] font-bold">للإيجار</span>
                    </label>
                  </div>
                </div>
                ...
                {/* الوصف */}
                <div className="flex flex-col gap-y-4 w-full">
                  <label className="text-emerald-600 text-[16px] font-semibold">الوصف</label>
                  <textarea
                    name="description"
                ...
                {/* تحميل الصور */}
                <div className="flex flex-col gap-y-4 w-full">
                  <label className="text-emerald-600 text-[16px] font-semibold">تحميل صور العقار</label>
                ...
                {/* مناطق تحميل الصور */}
                {[1, 2, 3].map((index) => (
                  <div key={index} className="relative bg-gray-100 w-[300px] h-64 flex flex-col items-center justify-center rounded-md mx-auto">
                    <input
                      accept="image/*"
                ...
                <button
                  type="submit"
                  className={mergedData.form?.submitButton?.className || "rounded-md bg-emerald-600 text-white font-semibold text-[16px] py-2 w-full md:w-[400px] mx-auto mb-10 hover:bg-emerald-700 transition-colors"}
                  style={{
