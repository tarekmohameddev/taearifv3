...
export default function CrmHeader({ onRefresh, onSettings }: CrmHeaderProps) {
  return (
    <div className="flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight">نظام إدارة علاقات العملاء (CRM)</h1>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
        ...
        <Button variant="outline" onClick={onRefresh} className="w-full sm:w-auto bg-transparent">
          <RefreshCw className="ml-2 h-4 w-4" />
          <span className="hidden sm:inline">تحديث البيانات</span>
          <span className="sm:hidden">تحديث</span>
        </Button>
        <Button variant="outline" onClick={onSettings} className="w-full sm:w-auto">
