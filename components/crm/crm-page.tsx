...
import { Users, TrendingUp, Target, Handshake, CheckCircle, Calendar, Phone, Clock, Star, Award, User, Shield } from "lucide-react"
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-xl sm:text-2xl font-bold">
            {customersData.filter((c: Customer) => c.pipelineStage !== "closed-lost").length}
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          <div className="text-xs text-muted-foreground">
            {((pipelineStages.find((s) => s.id === "2")?.value || 0) / 1000).toFixed(0)}ك ريال
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          <div className="text-xs text-muted-foreground">
            {((pipelineStats.find((s) => s.id === "negotiation")?.value || 0) / 1000).toFixed(0)}ك ريال
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          <div className="text-xs text-muted-foreground">
            {((pipelineStats.find((s) => s.id === "closed-won")?.value || 0) / 1000000).toFixed(1)}م ريال
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <div className="text-xl sm:text-2xl font-bold">{scheduledAppointments}</div>
          <div className="text-xs text-muted-foreground">{totalAppointments} إجمالي</div>
        </CardContent>
      </Card>
    </div>
  )
...
export default function CrmPage() {
  ...
  const pipelineStages = Array.isArray(rawPipelineStages) ? rawPipelineStages : []
    ...
    fetchRemindersData: async () => {
      try {
        ...
        if (remindersResponse.status === "success") {
          ...
          const transformedReminders = (remindersResponse.data || []).map((reminder: any) => ({
            id: reminder.id,
  ...
  const utilities = Utilities({
    ...
    if (stageElement) {
      ...
      stageElement.classList.add("animate-pulse", "ring-2", "ring-green-500", "scale-105")
      setTimeout(() => {
        stageElement.classList.remove("animate-pulse", "ring-2", "ring-green-500", "scale-105")
  ...
  const enhancedDragDrop = EnhancedDragDrop({
    ...
    onDrop: async (e, stageId) => {
      if (dragPreview && dragPreview.pipelineStage !== stageId) {
        ...
        dataHandler.updateCustomerStage(dragPreview.id, stageId)
          .then(success => {
            if (!success) {
              // Revert if API fails
              updateCustomerStage(dragPreview.id, originalStage)
              utilities.announceToScreenReader(`فشل في نقل العميل ${customerName}`)
            }
          })
          .catch(error => {
            // Revert if API fails
  ...
  const updateReminderInList = (updatedReminder: any) => {
    setRemindersData(prev => prev.map(reminder => 
      reminder.id === updatedReminder.id ? updatedReminder : reminder
    ))
  }

  const updateAppointmentInList = (updatedAppointment: any) => {
    setAppointmentsData(prev => prev.map(appointment => 
  ...
  const updateStageInList = (updatedStage: any) => {
    setPipelineStages(pipelineStages.map((stage: PipelineStage) => 
      stage.id === updatedStage.id ? updatedStage : stage
    ))
  }

  const removeStageFromList = (stageId: string) => {
    setPipelineStages(pipelineStages.filter((stage: PipelineStage) => stage.id !== stageId))
  }
  ...
  const handleCompleteReminder = async (reminder: any) => {
    try {
      const response = await axiosInstance.put(`/crm/customer-reminders/${reminder.id}`, {
        status: 'completed'
      })
      if (response.data.status === "success") {
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        ...
        <main className="flex-1 p-4 md:p-6">
          <div className="space-y-6">
            ...
            <EditAppointmentDialog onAppointmentUpdated={updateAppointmentInList} />
            <AddInteractionDialog />
