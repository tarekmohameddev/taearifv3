...
export default function KeyboardNavigation({
    ...
    if (focusedCustomer && focusedCustomer.id === customer.id) {
      if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
        ...
        const currentIndex = pipelineStages.findIndex(
          (s: PipelineStage) => s.id === focusedStage?.id,
        );
        const direction = e.key === "ArrowRight" ? 1 : -1;
        const newIndex = Math.max(
          0,
          Math.min(pipelineStages.length - 1, currentIndex + direction),
        );
  ...
  const moveCustomerToStage = (customer: Customer, targetStageId: string) => {
    ...
    const sourceStage = pipelineStages.find(
      (s: PipelineStage) => s.id === customer.pipelineStage,
    );
    const targetStage = pipelineStages.find(
      (s: PipelineStage) => s.id === targetStageId,
    );
