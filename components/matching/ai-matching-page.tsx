...
function calculateMatchScore(request: any, property: any) {
  ...
  maxScore += 25
  if (property.price >= request.budget.min && property.price <= request.budget.max) {
    score += 25
    reasons.push("السعر ضمن الميزانية المحددة")
  } else if (property.price < request.budget.max * 1.1 && property.price > request.budget.min * 0.9) {
    score += 15
  ...
  if (property.city === requestLocation[0] && property.district === requestLocation[1]) {
    score += 15
  ...
  maxScore += 15
  if (property.bedrooms >= request.bedrooms.min && property.bedrooms <= request.bedrooms.max) {
    score += 15
  ...
  maxScore += 10
  if (property.bathrooms >= request.bathrooms.min && property.bathrooms <= request.bathrooms.max) {
    score += 10
  ...
  } else if (property.size >= request.size.min * 0.9 && property.size <= request.size.max * 1.1) {
    score += 5
  ...
  if (matchingFeatures.length > 0) {
    const featureScore = Math.min(5, (matchingFeatures.length / request.features.length) * 5)
    score += featureScore
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                ...
                <p className="text-muted-foreground">ربط طلبات العملاء تلقائياً مع العقارات المناسبة من المخزون</p>
              </div>
              ...
              <Card className="lg:col-span-1">
                ...
                <CardContent className="space-y-3">
                  {loading ? (
                    ...
                    <div className="text-center py-4">
                      <div className="text-sm text-muted-foreground">لا توجد بيانات عملاء</div>
                    </div>
                  ) : (
                    customers.map((customer: any) => (
                      <Card
                        ...
                        className={`cursor-pointer transition-all hover:shadow-md ${
                          selectedRequest?.id === customer.id ? "ring-2 ring-purple-500" : ""
                        }`}
                        onClick={async () => {
                          try {
                            ...
                            const requestData = {
                              ...
                              customerAvatar: customer.assignedAgent?.avatar || "/placeholder.svg",
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-start gap-3">
                            <Avatar className="h-10 w-10">
                              <AvatarImage src={customer.assignedAgent?.avatar || "/placeholder.svg"} />
                              <AvatarFallback>
                                {customer.name
                                  ?.split(" ")
                                  ?.slice(0, 2)
                ...
                <CardContent>
                  {loadingProperties ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-lg font-medium mb-2">لا توجد مطابقات</h3>
                      <p className="text-sm text-muted-foreground">لم يتم العثور على عقارات تطابق متطلبات هذا العميل</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {matchingProperties.map((property: any, index: number) => (
                        <Card
                          key={property.id}
                          ...
                          onClick={async () => {
                            try {
                              ...
                              const matchDetails = await fetchMatchDetails(property.id);
                              ...
                              setSelectedMatch({
                                ...
                                matchingFeatures: matchDetails.match.matchedCriteria,
                        ...
                        >
                          <div className="flex flex-col sm:flex-row">
                            <div className="relative sm:w-48 h-48 sm:h-auto">
                              ...
                              />
                              <Badge className={`absolute top-2 left-2 ${getScoreColor(property.matchingScore)}`}>
                            ...
                            <div className="flex-1 p-4">
                              ...
                              <div className="mb-3">
                                <div className="flex items-center justify-between mb-1">
                                  <span className="text-xs font-medium">{getScoreLabel(property.matchingScore)}</span>
                                  <span className="text-xs text-muted-foreground">{property.matchingScore}%</span>
                                </div>
                                <Progress value={property.matchingScore} className="h-2" />
                              </div>
                ...
                {selectedMatch && (
                  <>
                    <DialogHeader>
                      ...
                      <DialogDescription>تحليل شامل لمدى توافق العقار مع متطلبات العميل</DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-6">
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-lg">نسبة التوافق</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="flex items-center gap-4">
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-2xl font-bold">{selectedMatch?.score || 0}%</span>
                                <Badge className={getScoreColor(selectedMatch?.score || 0)}>
                                  {getScoreLabel(selectedMatch?.score || 0)}
                                </Badge>
                              </div>
                              <Progress value={selectedMatch?.score || 0} className="h-3" />
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <div className="grid gap-6 md:grid-cols-2">
                        <Card>
                          ...
                          <CardContent className="space-y-3">
                            <img
                              src={selectedMatch?.property?.thumbnail || selectedMatch?.property?.featuredImage || "/placeholder.svg"}
                              alt={selectedMatch?.property?.title || selectedMatch?.property?.name || "عقار"}
                              className="w-full h-48 object-cover rounded-lg"
                            />
                            <div>
                              <h3 className="font-semibold text-lg">{selectedMatch?.property?.title || selectedMatch?.property?.name || "عقار غير محدد"}</h3>
                              <p className="text-sm text-muted-foreground flex items-center gap-1">
                                <MapPin className="h-3 w-3" />
                                {selectedMatch?.property?.address || "موقع غير محدد"}
                              </p>
                            </div>
                            <div className="grid grid-cols-2 gap-3 pt-2">
                              <div>
                                <div className="text-xs text-muted-foreground">النوع</div>
                                <div className="font-medium">{selectedMatch?.property?.type || selectedMatch?.property?.forRentOrSale || "غير محدد"}</div>
                              </div>
                              <div>
                                <div className="text-xs text-muted-foreground">السعر</div>
                                <div className="font-medium text-green-600">
                                  {selectedMatch?.property?.price?.toLocaleString() || selectedMatch?.property?.rentOrBuyAmount?.toLocaleString() || "غير محدد"} ريال
                                </div>
                              </div>
                              <div>
                                <div className="text-xs text-muted-foreground">غرف النوم</div>
                                <div className="font-medium">{selectedMatch?.property?.bedrooms || "غير محدد"}</div>
                              </div>
                              <div>
                                <div className="text-xs text-muted-foreground">الحمامات</div>
                                <div className="font-medium">{selectedMatch?.property?.bathrooms || selectedMatch?.property?.baths || "غير محدد"}</div>
                              </div>
                              <div>
                                <div className="text-xs text-muted-foreground">المساحة</div>
                                <div className="font-medium">{selectedMatch?.property?.size || selectedMatch?.property?.area || "غير محدد"} م²</div>
                              </div>
                              <div>
                                <div className="text-xs text-muted-foreground">الحالة</div>
                                <div className="font-medium">{selectedMatch?.property?.listingType || (selectedMatch?.property?.forRentOrSale === "rent" ? "للإيجار" : "للبيع") || "غير محدد"}</div>
                              </div>
                            </div>
                            <div>
                              <div className="text-xs text-muted-foreground mb-2">المميزات</div>
                              <div className="flex flex-wrap gap-1">
                                {(selectedMatch?.property?.features || selectedMatch?.property?.matchedCriteria || []).map((feature: string, idx: number) => (
                                  <Badge key={idx} variant="secondary" className="text-xs">
                                    {feature}
                                  </Badge>
                                ))}
                          ...
                          <CardContent className="space-y-3">
                            <div className="flex items-center gap-3">
                              <Avatar className="h-12 w-12">
                                <AvatarImage src={selectedRequest.customerAvatar || "/placeholder.svg"} />
                                <AvatarFallback>
                          ...
                          <div className="grid gap-2">
                            {(() => {
                              ...
                              if (selectedMatch?.reasons) {
                                ...
                                } else if (Array.isArray(selectedMatch.reasons)) {
                                  reasonsArray = selectedMatch.reasons;
                                }
                              } else if (selectedMatch?.matchingFeatures && Array.isArray(selectedMatch.matchingFeatures)) {
                                reasonsArray = selectedMatch.matchingFeatures;
                              }
                              
                              return reasonsArray.map((reason: string, idx: number) => (
                      ...
                      <div className="flex justify-end gap-2">
                        <Button 
                          variant="outline"
                          onClick={() => {
                            const phoneNumber = selectedMatch?.request?.phone || selectedMatch?.property?.phone;
                            if (phoneNumber) {
