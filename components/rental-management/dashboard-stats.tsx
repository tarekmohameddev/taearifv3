...
function ExpiringContractsDialog() {
  ...
  // فلترة العقود المنتهية خلال 30 يوم
  const expiringContracts = ongoingRentals.filter((rental: any) => {
    if (!rental.contract?.end_date) return false
    const endDate = new Date(rental.contract.end_date)
      ...
      <DialogContent className="sm:max-w-[1000px] max-h-[85vh] overflow-y-auto" dir="rtl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {expiringContracts.length === 0 ? (
            ...
            expiringContracts.map((rental: any) => {
              const endDate = new Date(rental.contract.end_date)
              const today = new Date()
              const diffTime = endDate.getTime() - today.getTime()
              const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24))
              ...
              return (
                <Card key={rental.id} className="hover:shadow-lg transition-all duration-300 border-l-4 border-l-yellow-500">
                  <CardContent className="p-6">
                    {/* Header */}
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold text-lg text-gray-900">{rental.tenant_name}</h3>
                          <p className="text-sm text-gray-500">عقد #{rental.contract?.id}</p>
                        </div>
                      </div>
                      <Badge className={`${
                        diffDays <= 7 ? 'bg-red-100 text-red-800 border-red-200' :
                        diffDays <= 14 ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                        'bg-gray-100 text-gray-800 border-gray-200'
                      }`}>
                        {diffDays} يوم متبقي
                      </Badge>
