    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-yellow-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-yellow-800">عقود تنتهي قريباً</p>
                  <p className="text-xs text-yellow-700 mt-1">{stats.expiringContracts} عقود تنتهي خلال 30 يوم</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-blue-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-blue-800">طلبات قيد المراجعة</p>
                  <p className="text-xs text-blue-700 mt-1">{stats.pendingRequests} طلب إيجار يحتاج للمراجعة</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-red-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-red-800">مدفوعات متأخرة</p>
                  <p className="text-xs text-red-700 mt-1">{stats.pendingPayments} مدفوعات متأخرة عن موعدها</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-orange-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-orange-800">طلبات صيانة</p>
                  <p className="text-xs text-orange-700 mt-1">{stats.maintenanceRequests} طلب صيانة قيد التنفيذ</p>
                </div>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>إحصائيات الأداء للشهر الحالي</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{stats.approvedRequests}</div>
              <div className="text-sm text-green-700">طلبات موافق عليها</div>
              <div className="text-xs text-green-600 mt-1">
                معدل الموافقة: {((stats.approvedRequests / stats.totalRequests) * 100).toFixed(1)}%
              </div>
            </div>

            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{stats.occupiedProperties}</div>
              <div className="text-sm text-blue-700">عقارات مؤجرة</div>
              <div className="text-xs text-blue-600 mt-1">
                معدل الإشغال: {((stats.occupiedProperties / stats.totalProperties) * 100).toFixed(1)}%
              </div>
            </div>

            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <div className="text-2xl font-bold text-purple-600">{stats.totalRevenue.toLocaleString()}</div>
              <div className="text-sm text-purple-700">إجمالي الإيرادات (ر.س)</div>
              <div className="text-xs text-purple-600 mt-1">
                متوسط الإيجار: {Math.round(stats.totalRevenue / stats.occupiedProperties).toLocaleString()} ر.س
              </div>
            </div>
          </div>
        </CardContent>
