...
export function PurchaseDetailPage({ requestId }: PurchaseDetailPageProps) {
  ...
  const getStageIcon = (status: string) => {
    ...
    if (status === "in-progress") return <Clock className="h-4 w-4 text-white" />
  ...
  const canTransition = (currentStage: string) => {
    ...
    return (
      ...
      request.stages[currentStage as keyof typeof request.stages].status === "in-progress"
    )
  }
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">تفاصيل طلب الشراء {request.requestNumber}</h1>
              <p className="text-gray-600">تم الإنشاء في {new Date(request.createdAt).toLocaleDateString("ar-SA")}</p>
            </div>
          </div>
          <div className="flex items-center space-x-2 space-x-reverse">
            <Badge className={getPriorityColor(request.priority)}>{getPriorityText(request.priority)}</Badge>
            <Badge variant="outline">{getStageText(request.currentStage)}</Badge>
          </div>
        </div>
        ...
        <Card className="mb-6">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-2xl font-bold text-blue-600">{request.progress}%</span>
            </div>
            ...
            <div className="flex justify-between">
              {stages.map((stage, index) => (
                <div key={stage.key} className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-gray-500 mt-1">{getStatusText(stage.status)}</span>
                </div>
              ))}
            </div>
          </CardContent>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between">
                <div>
                  ...
                  <p className="text-lg font-semibold text-yellow-600">{request.client.rating}/5</p>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-600">إجمالي المشتريات</Label>
                  <p className="text-lg font-semibold">{request.client.totalPurchases}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="text-lg font-semibold">{request.property.title}</p>
              </div>
              <div className="flex items-center space-x-2 space-x-reverse">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium text-gray-600">غرف النوم</Label>
                  <p className="text-sm">{request.property.bedrooms}</p>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-600">دورات المياه</Label>
              ...
              <div>
                <Label className="text-sm font-medium text-gray-600">المطور</Label>
            ...
            <Card key={stage.key}>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">{getStatusText(stage.status)}</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {request.stages[stage.key as keyof typeof request.stages].completedAt && (
                ...
                {request.stages[stage.key as keyof typeof request.stages].documents.length > 0 && (
                  <div>
                    <Label className="text-sm font-medium text-gray-600">المستندات</Label>
                    <div className="mt-2 space-y-1">
                      {request.stages[stage.key as keyof typeof request.stages].documents.map((doc, index) => (
                ...
                {request.stages[stage.key as keyof typeof request.stages].notes && (
                  <div>
                    <Label className="text-sm font-medium text-gray-600">ملاحظات</Label>
                    <p className="text-sm mt-1">{request.stages[stage.key as keyof typeof request.stages].notes}</p>
                  </div>
                )}
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            setTransitionForm((prev) => ({
                              ...prev,
                              documents: prev.documents.filter((doc) => doc !== req),
                            }))
                          }
                        }}
                      />
