...
export class PurchaseRequestsService {
  ...
  static async getPurchaseRequestStats(): Promise<{
    ...
    try {
      ...
      const stats = {
        total: data.length,
        pending: data.filter(
          (req: PurchaseRequest) => req.overall_status === "pending",
        ).length,
        in_progress: data.filter(
          (req: PurchaseRequest) => req.overall_status === "in_progress",
        ).length,
        completed: data.filter(
          (req: PurchaseRequest) => req.overall_status === "completed",
        ).length,
        by_priority: data.reduce(
          (acc: Record<string, number>, req: PurchaseRequest) => {
  ...
  static async transitionToNextStage(
    ...
    try {
      ...
      console.error(
        `Error transitioning stage for purchase request with ID ${id}:`,
        error,
      );
      throw error;
    }
  ...
  static async deletePurchaseRequest(
    id: string,
  ): Promise<DeletePurchaseRequestResponse> {
    const token = useAuthStore.getState().userData?.token;
    if (!token) {
