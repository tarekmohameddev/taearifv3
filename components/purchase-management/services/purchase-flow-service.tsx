...
export default function PurchaseFlowService() {
  ...
  const addRequestSchema = z.object({
    ...
    priority: z.enum(["low","medium","high","urgent"], { required_error: "الأولوية مطلوبة" }),
  ...
  // Field-level errors
  const [newRequestErrors, setNewRequestErrors] = useState<Record<string, string>>({})
  const [editRequestErrors, setEditRequestErrors] = useState<Record<string, string>>({})
  ...
  const mapApiPriorityToPriority = (priority: string): "low" | "medium" | "high" | "urgent" => {
    switch (priority) {
      case "عاجل":
        return "urgent"
      case "متوسطة":
  ...
  const mapStageStatus = (status: string | undefined): "completed" | "in_progress" | "pending" | "cancelled" => {
  ...
  const filteredRequests = requests.filter((request: any) => {
    const matchesSearch =
      ...
      (request.client.nameAr || request.client.name || "").includes(searchTerm) ||
  ...
  const analytics = {
    ...
    totalValue: requests.reduce((sum: any, req: any) => sum + req.totalValue, 0),
    totalPaid: requests.reduce((sum: any, req: any) => sum + req.paidAmount, 0),
    averageProgress: requests.reduce((sum: any, req: any) => sum + getStageProgress(req), 0) / requests.length,
    stageDistribution: {
      reservation: requests.filter((r: any) => r.currentStage === "reservation").length,
      contract: requests.filter((r: any) => r.currentStage === "contract").length,
      completion: requests.filter((r: any) => r.currentStage === "completion").length,
      receiving: requests.filter((r: any) => r.currentStage === "receiving").length,
    },
    riskDistribution: {
  ...
  function getStageProgress(request: PurchaseRequest): number {
    ...
    const currentStageKey = request.currentStage as keyof PurchaseRequest["stages"]
  ...
  function getStageRequirements(fromStage: string, toStage: string): StageRequirement[] {
    const requirements: { [key: string]: StageRequirement[] } = {
      "reservation-contract": [
        { id: "id_copy", label: "ID Copy", labelAr: "صورة الهوية", type: "document", required: true, completed: false },
        {
  ...
  function canTransitionToStage(request: PurchaseRequest, targetStage: string): boolean {
  ...
  function handleStageTransition(requestId: string, fromStage: string, toStage: string) {
    ...
    setTransitionForm({
      requirements: requirements.reduce((acc, req) => ({ ...acc, [req.id]: false }), {}),
      notes: "",
      documents: [],
      amount: 0,
  ...
  async function completeStageTransition() {
    const request = requests.find((r: any) => r.id === stageTransition.requestId)
    if (!request) return
    ...
    try {
      ...
      const transitionData = {
        ...
        expected_completion_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      ...
      setStageTransition({ isOpen: false, requestId: "", fromStage: "", toStage: "", requirements: [] })
      setTransitionForm({})
  ...
  function handleDeleteRequest(requestId: string) {
    ...
    openDeleteConfirmDialog(requestId, request.requestNumber, request.client.nameAr || request.client.name)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">تتبع ومتابعة عمليات شراء العقارات</p>
        </div>
              ...
              {filteredRequests.map((request: PurchaseRequest) => (
                <>
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2 space-x-reverse">
                        <Avatar className="h-6 w-6">
                          <AvatarImage src={request.client.avatar || "/placeholder.svg"} />
                          <AvatarFallback>{(request.client.nameAr || request.client.name || "").charAt(0)}</AvatarFallback>
                        </Avatar>
                        <span className="text-sm">{request.client.nameAr || request.client.name}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        <p className="text-sm font-medium">{request.property.title || request.property.titleAr || ""}</p>
                        <p className="text-xs text-muted-foreground">{request.property.location || request.property.locationAr || ""}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="space-y-1">
                        <Progress value={getStageProgress(request)} className="h-2 w-16" />
                        <span className="text-xs text-muted-foreground">{Math.round(getStageProgress(request))}%</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        {(() => {
                          const total = (request as any).totalValue ?? (typeof request.property.price === "number" ? request.property.price : 0)
                          const paid = (request as any).paidAmount ?? 0
                          return (
                            <>
                              <p className="text-sm font-medium">{(Number(total) / 1000).toLocaleString()}ك</p>
                              <p className="text-xs text-muted-foreground">مدفوع: {(Number(paid) / 1000).toLocaleString()}ك</p>
                            </>
                          )
                        })()}
                      </div>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-1 space-x-reverse">
                        ...
                        <Button
                          ...
                          disabled={isDeletingRequest && deleteConfirmDialog.requestId === request.id}
                        >
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setEditRequestForm({ ...editRequestForm, clientNameAr: e.target.value })}
                  placeholder="أدخل اسم العميل بالعربية"
                />
                {editRequestErrors.clientNameAr && (
                  <p className="text-xs text-red-500 mt-1">{editRequestErrors.clientNameAr}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setEditRequestForm({ ...editRequestForm, clientEmail: e.target.value })}
                  placeholder="client@example.com"
                />
                {editRequestErrors.clientEmail && (
                  <p className="text-xs text-red-500 mt-1">{editRequestErrors.clientEmail}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setEditRequestForm({ ...editRequestForm, clientPhone: e.target.value })}
                  placeholder="+966 50 123 4567"
                />
                {editRequestErrors.clientPhone && (
                  <p className="text-xs text-red-500 mt-1">{editRequestErrors.clientPhone}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setEditRequestForm({ ...editRequestForm, clientNationalId: e.target.value })}
                  placeholder="1234567890"
                />
                {editRequestErrors.clientNationalId && (
                  <p className="text-xs text-red-500 mt-1">{editRequestErrors.clientNationalId}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setEditRequestForm({ ...editRequestForm, selectedPropertyId: value })}
                  disabled={loadingProperties}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={loadingProperties ? "جاري تحميل العقارات..." : "اختر العقار"} />
                  </SelectTrigger>
                  <SelectContent>
                        ...
                        <SelectItem key={property.id} value={property.id.toString()}>
                          <div className="flex flex-col">
                            ...
                            <span className="text-sm text-gray-500">
                              {property.type} - {property.price} ر.س - {property.area} م²
                            </span>
                          </div>
                        </SelectItem>
                      ))
                ...
                {editRequestErrors.selectedPropertyId && (
                  <p className="text-xs text-red-500 mt-1">{editRequestErrors.selectedPropertyId}</p>
                )}
                {editRequestForm.selectedPropertyId && Array.isArray(properties) && (
                    ...
                    {(() => {
                      ...
                      return selectedProperty ? (
                        <div className="bg-gray-50 p-3 rounded-lg">
                          ...
                          <p><strong>السعر:</strong> {selectedProperty.price} ر.س</p>
                          <p><strong>المساحة:</strong> {selectedProperty.area} م²</p>
                          <p><strong>الغرف:</strong> {selectedProperty.beds}</p>
                          <p><strong>الحمامات:</strong> {selectedProperty.bath}</p>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setEditRequestForm({ ...editRequestForm, priority: value })}
                >
                  <SelectTrigger>
          ...
          <div className="flex justify-end space-x-2 space-x-reverse mt-6">
            <Button variant="outline" onClick={() => setIsEditDialogOpen(false)} disabled={isUpdatingRequest}>
              إلغاء
            </Button>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewRequestForm({ ...newRequestForm, clientEmail: e.target.value })}
                  placeholder="client@example.com"
                />
                {newRequestErrors.clientEmail && (
                  <p className="text-xs text-red-500 mt-1">{newRequestErrors.clientEmail}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewRequestForm({ ...newRequestForm, clientPhone: e.target.value })}
                  placeholder="+966 50 123 4567"
                />
                {newRequestErrors.clientPhone && (
                  <p className="text-xs text-red-500 mt-1">{newRequestErrors.clientPhone}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewRequestForm({ ...newRequestForm, clientNationalId: e.target.value })}
                  placeholder="1234567890"
                />
                {newRequestErrors.clientNationalId && (
                  <p className="text-xs text-red-500 mt-1">{newRequestErrors.clientNationalId}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setNewRequestForm({ ...newRequestForm, selectedPropertyId: value })}
                  disabled={loadingProperties}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={loadingProperties ? "جاري تحميل العقارات..." : "اختر العقار"} />
                ...
                {newRequestErrors.selectedPropertyId && (
                  <p className="text-xs text-red-500 mt-1">{newRequestErrors.selectedPropertyId}</p>
                )}
                {newRequestForm.selectedPropertyId && Array.isArray(properties) && (
                  <div className="text-sm text-gray-600">
                    {(() => {
                      const selectedProperty = properties.find(p => p.id.toString() === newRequestForm.selectedPropertyId)
                      return selectedProperty ? (
          ...
          <div className="space-y-4 mt-6">
            <div className="space-y-2">
              ...
              {editRequestErrors.priority && (
                <p className="text-xs text-red-500 mt-1">{editRequestErrors.priority}</p>
              )}
              {newRequestErrors.priority && (
                <p className="text-xs text-red-500 mt-1">{newRequestErrors.priority}</p>
              )}
            </div>
          ...
          <div className="flex justify-end space-x-2 space-x-reverse mt-6">
            <Button variant="outline" onClick={() => setIsAddDialogOpen(false)} disabled={isCreatingRequest}>
              إلغاء
            </Button>
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setStageTransition({ isOpen: false, requestId: "", fromStage: "", toStage: "", requirements: [] })
        }
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              الانتقال من {getStageNameAr(stageTransition.fromStage)} إلى {getStageNameAr(stageTransition.toStage)}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {getStageRequirements(stageTransition.fromStage, stageTransition.toStage).map((req) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setTransitionForm({ ...transitionForm, amount: Number.parseInt(e.target.value) })}
                  placeholder="0"
                />
              </div>
            </div>
          ...
          <div className="flex justify-end space-x-2 space-x-reverse mt-6">
            ...
            <Button onClick={completeStageTransition} disabled={isTransitioningStage}>
      ...
      <Dialog open={deleteConfirmDialog.isOpen} onOpenChange={cancelDeleteRequest}>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{deleteConfirmDialog.requestNumber}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-medium text-gray-700">اسم العميل:</span>
                  <span className="text-gray-900">{deleteConfirmDialog.clientName}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-yellow-800 text-sm">
                <strong>ملاحظة مهمة:</strong> جميع البيانات المرتبطة بهذا الطلب بما في ذلك المراحل والمستندات والملاحظات ستتم إزالتها نهائياً.
              </p>
            </div>
          </div>
