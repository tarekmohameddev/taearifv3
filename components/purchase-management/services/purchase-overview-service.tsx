    ...
    const fetchOverviewData = async () => {
      ...
      try {
        ...
        const stats = {
          ...
          pendingRequests: requests.filter(req => req.overall_status === "pending").length,
          approvedRequests: requests.filter(req => req.overall_status === "completed").length,
          rejectedRequests: 0, // Not provided in API
          activeContracts: requests.filter(req => req.overall_status === "in_progress").length,
          completedPurchases: requests.filter(req => req.overall_status === "completed").length,
          totalInvestment: requests.reduce((sum, req) => sum + parseFloat(req.budget_amount), 0),
          monthlyInvestment: requests.reduce((sum, req) => sum + parseFloat(req.budget_amount), 0) * 0.1, // 10% of total
          pendingPayments: requests.filter(req => req.overall_status === "pending").length,
          scheduledInspections: 5, // Not provided in API, using default
          availableProperties: 45, // Not provided in API, using default
          underNegotiation: requests.filter(req => req.overall_status === "in_progress").length,
        }
        ...
        const transformedActivity: RecentActivity[] = recentRequests.map((request) => ({
          ...
          dateHijri: new Date(request.created_at).toLocaleDateString("ar-SA-u-ca-islamic"),
          status: request.overall_status,
          statusAr: request.overall_status === "pending" ? "قيد المراجعة" : 
                   request.overall_status === "in_progress" ? "قيد التنفيذ" : "مكتمل",
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">ملخص شامل لجميع أنشطة شراء العقارات</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">الاستثمار الشهري</CardTitle>
            <DollarSign className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(stats.monthlyInvestment / 1000000).toFixed(1)}م ر.س</div>
            <div className="flex items-center space-x-2 space-x-reverse text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">المعاينات المجدولة</CardTitle>
                ...
                <div key={activity.id} className="flex items-start space-x-3 space-x-reverse">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium">{activity.descriptionAr}</p>
                      <Badge className={`text-xs ${getStatusColor(activity.status)}`}>{activity.statusAr}</Badge>
                    </div>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-green-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-green-800">فرص استثمارية جديدة</p>
                  <p className="text-xs text-green-700 mt-1">{stats.availableProperties} عقار متاح للشراء في المنطقة</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-blue-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-blue-800">معاينات مجدولة</p>
                  <p className="text-xs text-blue-700 mt-1">{stats.scheduledInspections} معاينات مجدولة هذا الأسبوع</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-yellow-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-yellow-800">طلبات تحتاج مراجعة</p>
                  <p className="text-xs text-yellow-700 mt-1">{stats.pendingRequests} طلب شراء يحتاج للمراجعة</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 space-x-reverse p-3 bg-purple-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-purple-800">تفاوضات جارية</p>
                  <p className="text-xs text-purple-700 mt-1">{stats.underNegotiation} عقارات قيد التفاوض</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{(stats.totalInvestment / 1000000).toFixed(1)}م</div>
              <div className="text-sm text-blue-700">إجمالي الاستثمارات (ر.س)</div>
              <div className="text-xs text-blue-600 mt-1">
                متوسط الاستثمار: {(stats.totalInvestment / stats.completedPurchases / 1000).toFixed(0)}ك ر.س
              </div>
            </div>
            ...
            <div className="text-center p-4 bg-green-50 rounded-lg">
              ...
              <div className="text-xs text-green-600 mt-1">
                معدل النجاح: {((stats.completedPurchases / stats.totalRequests) * 100).toFixed(1)}%
              </div>
            </div>

            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <div className="text-2xl font-bold text-orange-600">{stats.activeContracts}</div>
              <div className="text-sm text-orange-700">عقود نشطة</div>
              <div className="text-xs text-orange-600 mt-1">
                قيد التنفيذ: {((stats.activeContracts / stats.totalRequests) * 100).toFixed(1)}%
              </div>
            </div>
          </div>
        </CardContent>
