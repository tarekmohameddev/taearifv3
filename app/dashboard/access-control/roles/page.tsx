...
export default function RolesPage() {
  ...
  const [selectedRolePermissions, setSelectedRolePermissions] = useState<Record<string, boolean>>({});
  ...
  const [editSelectedRolePermissions, setEditSelectedRolePermissions] = useState<Record<string, boolean>>({});
  ...
  const handleCreateRole = async () => {
    ...
    try {
      ...
      setCreateRoleError(
        error.response?.data?.message || "فشل في إنشاء الدور",
      );
    } finally {
  ...
  const handleUpdateRole = async () => {
    ...
    try {
      ...
      setEditRoleError(
        error.response?.data?.message || "فشل في تحديث الدور",
      );
    } finally {
  ...
  const deleteRole = async () => {
    ...
    try {
      ...
      setDeleteRoleError(
        error.response?.data?.message || "فشل في حذف الدور",
      );
    } finally {
                  ...
                  <CardContent>
                    {roleDetailsLoading ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <Button
                          onClick={() => selectedRole && fetchRoleDetails(selectedRole.id)}
                          variant="outline"
                        >
                          إعادة المحاولة
                        </Button>
