...
import { Home, DollarSign, AlertTriangle, TrendingUp, User, LogOut } from "lucide-react";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { Avatar, AvatarFallback } from "@/components/ui/avatar";

export default function OwnerDashboard() {
  const { ownerData, logout, ownerIsLogged, fetchOwnerData, initializeFromStorage } = useOwnerAuthStore();
  ...
  const extractTenantId = (host: string): string | null => {
    ...
    if (isOnBaseDomain) {
      console.log("🔍 Dashboard: Host is base domain, not tenant-specific:", host);
      return null;
    }
    ...
    if (isDevelopment && host.includes(localDomain)) {
      ...
      if (parts.length > 1 && parts[0] !== localDomain) {
        ...
        console.log("🔍 Dashboard: Potential tenant ID (local):", potentialTenantId);

        if (!reservedWords.includes(potentialTenantId.toLowerCase())) {
          console.log("✅ Dashboard: Valid tenant ID (local):", potentialTenantId);
          return potentialTenantId;
        } else {
          console.log("❌ Dashboard: Reserved word (local):", potentialTenantId);
        }
      }
    }
    ...
    if (!isDevelopment && host.includes(productionDomain)) {
      ...
      if (parts.length > 2) {
        ...
        if (domainPart === productionDomain) {
          console.log("🔍 Dashboard: Potential tenant ID (production):", potentialTenantId);

          if (!reservedWords.includes(potentialTenantId.toLowerCase())) {
            console.log("✅ Dashboard: Valid tenant ID (production):", potentialTenantId);
            return potentialTenantId;
          } else {
            console.log("❌ Dashboard: Reserved word (production):", potentialTenantId);
          }
        }
      }
    }
          ...
          {dashboardData?.summary_cards && (
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{dashboardData.summary_cards.total_properties}</div>
                </CardContent>
              </Card>

              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">الإيجار المستحق</CardTitle>
                  <DollarSign className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold text-blue-600">{dashboardData.summary_cards.due_rent} ريال</div>
                </CardContent>
              </Card>

              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">الإيجار المتأخر</CardTitle>
                  <AlertTriangle className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold text-red-600">{dashboardData.summary_cards.overdue_rent} ريال</div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-green-600">{dashboardData.summary_cards.collection_rate}%</div>
                </CardContent>
              </Card>
            </div>
          )}
                  ...
                  <TableBody>
                    {dashboardData.properties_table.map((item: any, index: number) => (
                      <TableRow key={index}>
                        <TableCell>
                          <div className="flex items-center space-x-3 gap-x-3 ">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-gray-500">الوحدة: {item.property.unit_number}</div>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                          ...
                          {item.building.name && (
                            <div className="text-sm text-gray-500">{item.building.name}</div>
                          )}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {item.due_date ? new Date(item.due_date).toLocaleDateString('ar-US') : '-'}
                        </TableCell>
                        <TableCell>
                          {item.last_payment ? new Date(item.last_payment).toLocaleDateString('ar-US') : '-'}
                        </TableCell>
                        <TableCell>
                          <Badge 
                            variant={item.status.color === 'red' ? 'destructive' : item.status.color === 'green' ? 'default' : 'secondary'}
                          >
                            {item.status.text}
                          </Badge>
                        </TableCell>
