...
pm.test("Response contains success message", function () {
    if (pm.response.code === 302) {
        pm.expect(pm.response.headers.get("Location")).to.include("/admin/credit-packages");
    }
});
...
pm.environment.set("test_credits_per_message", Math.floor(Math.random() * 5) + 1);
pm.environment.set("test_price_per_credit", (Math.random() * 0.1 + 0.01).toFixed(4));
```
...
pm.test("Response contains success message", function () {
    if (pm.response.code === 302) {
        pm.expect(pm.response.headers.get("Location")).to.include("/admin/marketing-channel-pricing");
    }
});
